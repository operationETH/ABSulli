...
def library_recently_played_items(db: Session, library: Library, limit: int = 20) -> list[dict]:
    ...
    for row in rows:
        ...
        if dedupe_key in seen:
            continue
        seen.add(dedupe_key)
        media_item = db.query(MediaItem).filter(MediaItem.abs_item_id == item_id).first() if item_id else None
        items.append(
            {
                "abs_item_id": item_id,
...
def author_item_ids(db: Session, author_name: str) -> list[str]:
    return [book.abs_item_id for book in author_items(db, author_name) if book.abs_item_id]
